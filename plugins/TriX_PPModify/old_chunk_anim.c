...
int ppmodify_anim_subchunk_dump ( t_workspace *ws, int start, int length, t_treenode *node )
{
	subchunk_hdr header;
	...
	if ( !ws || !node || !v_valid ( ws, start ) )
		PLUGIN_ERROR ( "( !ws || !node || !v_valid ( ws, start ) ) failed" );
	...
	if ( header.length > length )
		PLUGIN_ERROR ( "header.length > length" );

	if ( header.length > v_get_size ( ws ) )
		PLUGIN_ERROR ( "header.length > filesize" );

	buffer = malloc ( header.length - 0x10 + 1 );
	if ( !buffer )
		PLUGIN_ERROR ( "failed allocating buffer" );
	...
	treenode_set_name ( node, "SUBCHUNK" );
		...
		ppmodify_anim_subchunk_dump_anims ( headernode, ws );

	return E_OK;
}

int ppmodify_anim_subchunk_dump_anims ( t_treenode *headernode, t_workspace *ws )
{
	int entries = 0;
	int entry = 0;
	int pos = 0;
	...
	entry = 0;
	while ( entry < entries && v_valid ( ws, pos + 0x0C ) )
	{
	...
	while ( entry < entries && v_valid ( ws, pos + 0x0C ) )
	{
		...
		treenode_set_name ( worknode, "ANIM" );
			...
			if ( entry < entries )
				size = anim_entries[entry+1].offset - anim_entries[entry].offset;
			else
				size = v_get_size ( ws ) - pos - anim_entries[entry].offset;

			buffer = malloc ( size );
			v_memcpy_get ( ws, buffer, anim_entries[entry].offset, size );
			childnode = treenode_addchild ( worknode );
			treenode_set_name ( childnode, "DUMP" );
			treenode_set_content_data_binary ( childnode, buffer, size );
