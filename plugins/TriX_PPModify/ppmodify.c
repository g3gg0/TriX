...
OPT_BEGIN
	...
	OPTION( OPT_BOOL, "PPM header ver (0=normal or 1=needed for 2610)", "ppm_header_ver", &ppm_header_ver )
//	OPTION( OPT_INT, "Compression factor (0-31) each step doubles the calculation time", "factor", &bitmask_depth )
//	OPTION( OPT_BOOL, "Auto resize workspace if it is too small", "auto_resize", &auto_resize )
...
//	OPTION( OPT_STR,  "String Test Variable", "teststring", &teststring )
	OPTION( OPT_BOOL, "Handle ANIM as unknown chunk", "anim_as_unknown", &anim_as_unknown )
	OPTION( OPT_BOOL, "Handle TEXT as unknown chunk", "text_as_unknown", &text_as_unknown )
OPT_FINISH


#define PPMODIFY_TITLE "PPModify"
#define PPMODIFY_MAJOR 0
#define PPMODIFY_MINOR 5
...
unsigned int ppmodify_ppm_chunk_dump ( t_workspace *ws, unsigned int start, t_treenode *node )
{
	...
	do
	{
		...
		if ( !strcmp ( "TEXT", header.name ) && !text_as_unknown )
			ret = ppmodify_text_subchunk_dump ( ws, address, avail, worknode );
...
{
	ppmodify_ppm_header_dump ( ws, start );
	return E_OK;
}

t_treenode *ppmodify_dump_xml ( t_workspace *ws, unsigned int start )
{
  	return ppmodify_ppm_header_dump_xml ( ws, start );
}
...
unsigned int ppmodify_build_ppm_chunk ( t_treenode *node )
{
	...
	while ( child )
	{
		...
		alignedlength = worklength;
		while ( alignedlength % 4 )
			alignedlength++;
...
unsigned int ppmodify_build_ppm ( t_workspace *ws, unsigned int start, unsigned int max_length, t_treenode *node )
{
	...
	memset ( header.ppm_string, 0x00, 0x24 );
	memset ( header.type, 0x00, 0x04 );
	memset ( header.version, 0x00, 0x04 );
	...
	while ( child )
	{
		if ( ppmodify_build_ppm_chunk ( child ) != E_OK )
		{
	...
	//v_memcpy_put ( tmp_ws, 0x00, "PPM", 0x04 );
