#ifndef __TRIX_HW_MCUPLL_C__
#define __TRIX_HW_MCUPLL_C__
...
unsigned int hwemu_mcupll_handle ( t_workspace *ws, unsigned int address, unsigned char type, unsigned int value )
{
	int okay = 0;
	int read = 1;
	int write = 0;
	unsigned int data = 0;
	unsigned char *sect = "MCUPLL";
	unsigned char *desc = "";
	unsigned int offset = address - MCUPLL_BASE;
	...
	if ( type & MODE_WRITE )
	{
		...
		if ( verbosity & HWEMU_VERB_MCUPLL )
