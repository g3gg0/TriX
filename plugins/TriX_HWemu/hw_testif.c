#ifndef __TRIX_HW_TESTIF_C__
#define __TRIX_HW_TESTIF_C__
...
unsigned int hwemu_testif_handle ( t_workspace *ws, unsigned int address, unsigned char type, unsigned int value )
{
	int okay = 0;
	int read = 1;
	int write = 0;
	unsigned int data = 0;
	unsigned char *sect = "TESTIF";
	unsigned char *desc = "";
	unsigned int offset = address - TESTIF_BASE;
	...
	if ( type & MODE_WRITE )
	{
		...
		if ( verbosity & HWEMU_VERB_TESTIF )
