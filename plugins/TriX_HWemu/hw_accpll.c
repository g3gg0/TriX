#ifndef __TRIX_HW_ACCPLL_C__
#define __TRIX_HW_ACCPLL_C__
...
unsigned int hwemu_accpll_handle ( t_workspace *ws, unsigned int address, unsigned char type, unsigned int value )
{
	int okay = 0;
	int read = 1;
	int write = 0;
	unsigned int data = 0;
	unsigned char *sect = "ACCPLL";
	unsigned char *desc = "";
	unsigned int offset = address - ACCPLL_BASE;
	...
	if ( type & MODE_WRITE )
	{
		...
		if ( verbosity & HWEMU_VERB_ACCPLL )
