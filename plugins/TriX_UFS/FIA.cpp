...
TFIA::TFIA(unsigned char *fia, unsigned int length)
{
  handle = -1;
  dataBin.ptr = NULL;
  dataSec.ptr = NULL;
  ...
  hdr.type = fia[0];
  hdr.length = (fia[1] << 24) | (fia[2] << 16) | (fia[3] << 8) | fia[4];
  hdr.sections = (fia[5] << 24) | (fia[6] << 16) | (fia[7] << 8) | fia[8];
  ...
  case FIA_FLASH:
    ...
    if( dataSec.size > 0 )
    {
	  ...
	  memcpy ( dataSec.ptr, &fia[9], dataSec.size );
    }

    dataBin.size = length - hdr.length - 5;
    if( dataBin.size > 0 )
    {
      dataBin.ptr = (unsigned char *)malloc( dataBin.size );
	  memcpy ( dataBin.ptr, &fia[9 + dataSec.size], dataBin.size );
    }
  }
}
...
int TFIA::CheckFlashID( int flashid )
{
  ...
  {
    if( strstr( (const char*)data, " BT " ) != NULL )
