 ...
 *
 * $Id: http_lib.h,v 1.3 2007/08/12 00:50:19 geggo Exp $
 *
 */
...
http_retcode http_put ( char *filename, char *data, unsigned int length, unsigned int overwrite, char *type );
http_retcode http_get ( char *filename, char **pdata, unsigned int *plength, char *typebuf );
http_retcode http_parse_url ( char *url, char **pfilename );
http_retcode http_delete ( char *filename );
http_retcode http_head ( char *filename, unsigned int *plength, char *typebuf );
