/************************************************************************
 ...
 File:initial.h
 Desc:data initialization routines
...
static int
scsReadDirectInitialization ( scSymbol * deli, scType * type, bool packed, pmem * dest, int type_size, scSymbol * sym )
{
    _value val;
...
//        j=4;
                    ...
                    if ( sym )
                    {
                        char *s1 = malloc (len+2); //scStrdup ( s );
						memcpy ( s1, s, len );
						s1[len] = '\000';
                        init_start = act.consts.pos;    //inits will be earlier by 4 bytes.
                        scvalConstantString ( &val2, s1, len+1 );
//           pushstr_pmem(&act.consts,s, strlen (s));
                        Gen_Opcode ( opvcpuPUSH, &val2, NULL );
                    }
                    else
                    {
                        pushstr_pmem ( &init_end, s, len );
						deb0 ( "\n" );
...
/*as STRING*/
                ...
                pushstr_pmem ( dest, data, strlen (data) );
/*      {char temp[100];
         ...
         {push_pmem(dest,i,temp);i=0;}
            ...
            else if ( lexeq ( "{" ) )
            {
                ...
                for ( ; i < tabsize; i++ )
                {
					...
					{
		                push_pmem ( dest, inner_type_size, &val.adr.val );
						j = inner_type_size;
					}
...
/*******************************************Primitive*/
        default:
        {
            ...
            if ( ip != act.ip && !deli )
            {
                //TODO:check special field if this is allowed
                if ( 0 )
                    serr ( scErr_Declaration, "Illegal initializer" );
            }
            switch ( sctypGetValType ( type ) )
            ...
            {
                deb0 ( "Initialization from immidiate\n" );
...
//   return 0;
            ...
            {                   //to heap
                ...
                else
                {
                    ...
                    else if ( val.adr.flags & adrIMPORTED )
                    {
...
///////////////////////////////////////////////////////////////////////////
void
sciReadInitialization ( scSymbol * deli, scSymbol * sym )
{                               //_value val;
    _value val1, val2;
    int size, type_size;
    ...
    if ( sctypIsFunction ( &type ) )
        serr2 ( scErr_Declaration, "Declaring function `%s' with `='", sym->name );
    Advance;
...
/****************STACK**********************/
    ...
    {
        int ip = 0;
		int zip = 0;   //save, where we were in code
		int saved_length = 0;
...
/*                ip = act.ip - ip;
                act.ip -= ip;*/
                code = scAlloc ( act.ip - ip );
                memcpy ( code, act.code.mem + ip, act.ip - ip );

				saved_length = act.ip - ip;
                zip = act.ip;
				act.ip = ip;
            }
            ...
            if ( code )         //restore what was generated in Gen_Opcode
            {
                ...
                push_pmem ( &act.code, saved_length, code );
                act.ip += saved_length;
    ...
    else
    {
        ...
        {                       //size=0 - nothing to copy!
            pop_pmem ( &act.inits, 8 ); //cancel previous init
            deb0 ( "Back - no data to copy\n" );
        }
        if ( init_end.pos )
        {
#if DEBUGLEVEL==0
