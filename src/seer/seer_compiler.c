...
static void
scsReadExp ( _value * val, bool RequiredExp )
{
    Advance;
...
///////////////////////////////////////////////////////////////////////////
static void
scsReadExp_PreserveRegs ( _value * val, bool RequiredExp )
{
    RegsTable p;
    //scget_lexema(ST);//mus
    Advance;
...
///////////////////////////////////////////////////////////////////////////
void
sciCountFunctionParams ( scSymbol * sym, scSymbol * in_struct, int Add_Params_To_Dict )
{
...
static int _fdFS;
static void
scsymCountAndDestruct ( scSymbol * sym )
{
...
void
sciAdjustScriptConstructor ( int level )
{
    Gen_Align (  );
    switch ( level )
    {
...
///////////////////////////////////////////////////////////////////////////
static void
scsReadFunctionBody ( scSymbol * deli, scSymbol * sym, int previous_address, scSymbol * in_struct )
{
...
*/
void
sciReadDefinition ( scSymbol * deli, scSymbol * sym, scSymbol * in_struct )
{
    _value val;
    ...
    do
    {
        ...
        if ( first )
        {
            if ( ( ( sym->type.flags & typTYPE ) == typUSERDEFINED ) && ( ( sym->type.main->type.flags & typTYPE ) == typSTRUCT || ( sym->type.main->type.flags & typTYPE ) == typSCRIPT ) )
            {                   //deb1("here we are %s\n",sym->type.main->name);
                if ( !deli && sym->type.main != in_struct && !sym->type.main->parent )
                {
                    scSymbol *psc;
        ...
        if ( !in_struct )
        {
            if ( !deli )
            {
                ...
                else if ( scdicFoundSymbol ( SC_symbols->first, sym->name ) )
                {
                    serr2 ( scErr_Declaration, "Symbol `%s' already defined", sym->name );
                }
            }
            else
                prev_sm = NULL;
        ...
        if ( sctypIsFunction ( &sym->type ) )
        {
            ...
            {                   //function had a prototype
                ...
                if ( !sctypIsFunction ( &prev_sm->type ) )
                    serr2 ( scErr_Declaration, "Symbol `%s' already defined not as a function", prev_sm->name );
        ...
        { /* TODO: stack corruption with char[x] in code ?! scsRoundTo32bit seemed to help? */
            if ( sctypIsReference ( &sym->type ) )
                serr ( scErr_Declaration, "reference without initializer" );
            if ( deli && !( sym->type.flags & typSTATIC ) )
            {                   //stack
                _value vt1, vt2;
                int i = scsRoundTo32bit ( sctypSizeOf ( &sym->type ));
        ...
        if ( !lexeq ( "," ) && !lexeq ( ";" ) && !lexeq ( "}" ) )
		{
			if ( ST->lexema[0] != '"' )
				...
				serr2 ( scErr_Parse, parse_error_concatstring, ST->lexema )
		}
    }
    while ( !( lexeq ( ";" ) || lexeq ( "}" ) ) );
...
do_switch ( scSymbol * deli )
{
    ...
    Advance;
    if ( !lexeq ( "(" ) )
        serr2 ( scErr_Parse, "`%s' instead of `(' required after switch", ST->lexema );
    scsReadExp ( &val, true );
        ...
        else if ( lexeq ( "default" ) )
        {
            ...
            do
            {
                ...
                else if ( lexeq ( "break" ) )
                {
                    ...
                    loop = 0;
					/* 
							ip_table[count - 1] = act.ip;

						count-1 ? count wasnt increased before!
						copy paste error by original author? 
						will cause break in a default switch to cause trouble!
					*/

                    ip_table[count] = act.ip;   //mark ending jump!
                    Gen_Opcode ( opvcpuJMP, scvalSetINT ( &vt2, 0 ), NULL );    //end
                }
                else if ( lexeq ( "case" ) || lexeq ( "default" ) )
                {
...
///////////////////////////////////////////////////////////////////////////
static void
scsReadOneStatement ( scSymbol * deli )
{
    static int lpro = -1;
    scSymbol *sym;
    ...
    {                           //debprintf("%s_",ST->lexema);
        ...
        {                       //scPosition pos;
            ...
            {
                scget_back ( ST );
                serr ( scErr_Declaration, "`;' expected after import declaration" );
            }
         ...
         if ( lexeq ( "return" ) )
        {
            ...
            if ( sctypGetValType ( &function->type ) != valVOID )
            {
                ...
                else if ( !( val.adr.flags & adrREGISTER && val.adr.address == 0 && !( val.adr.flags & adrPOINTER ) ) ) //Output via a0
                {
        ...
        if ( lexeq ( "if" ) )
        {
            ...
            Advance;
            if ( !lexeq ( "(" ) )
                serr2 ( scErr_Parse, "`%s' instead of `(' required after if", ST->lexema );
            scsReadExp ( &val, true );
            scsControlPoint (  );
            if ( !lexeq ( ")" ) )
                serr ( scErr_Parse, "`)' missing" );
        ...
        if ( lexeq ( "for" ) )
        {
            ...
            Advance;
            if ( !lexeq ( "(" ) )
                serr2 ( scErr_Parse, "`%s' instead of `(' required after for", ST->lexema );
        ...
        if ( lexeq ( "while" ) )
        {
            ...
            Advance;
            if ( !lexeq ( "(" ) )
                serr2 ( scErr_Parse, "`%s' instead of `(' required after while", ST->lexema );
            Gen_Align (  );
            sip = act.ip;
            scsReadExp ( &val, true );
            if ( !lexeq ( ")" ) )
        ...
        if ( lexeq ( "do" ) )
        {
            ...
            Advance;
            if ( !lexeq ( "while" ) )
                serr2 ( scErr_Parse, "while expected instead of '%s'", ST->lexema );
            Advance;
            if ( !lexeq ( "(" ) )
                serr2 ( scErr_Parse, "`%s' instead of `(' required after while", ST->lexema );
            scsReadExp ( &val, true );
...
///////////////////////////////////////////////////////////////////////////
static void
scsReadStatement ( scSymbol * deli )
{
...
///////////////////////////////////////////////////////////////////////////
scScript
scCompile ( Source_Text * ST_, int *length )
{                               //int i;
    ...
    ST = ST_;
    {
        ...
        for ( i = 0; i <= ST->num_lines; i++ )
        {
//			printf ( "%2d>%s:%i %s\n", i + 1, ST->filename[i], ST->linenum[i], ST->lines[i] );
            deb2 ( "%2d>%s\n", i + 1, ST->lines[i] );
        }
        // getchar();
    }
    ...
    //***********The Compilation begins*****************************
    {
        ...
        //HEADer
        {
            ...
            i = hdrHeaderSizeOf;
            push_pmem ( &act.final, 4, &i );    //hdrHeaderSize,4
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrSymbolOffset - in code,5
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrCodeOffset - in code,6
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrDataOffset - in data,7, NOT USED!
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrConstsOffset,8
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrInitOffset,9
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrCodeSize,10
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrDataSize,11
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrConstsSize,12
            i = act.stacksize;
            push_pmem ( &act.final, 4, &i );    //hdrStackSize,13
            act.stacksize = old_stack_size;
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrImportedOffset,14
            i = 0;
            push_pmem ( &act.final, 4, &i );    //hdrConstructOffset - in code,15
            i = act.code.pos;
            push_pmem ( &act.final, 4, &i );    //hdrDestructOffset - in code,16
            i = CHARS2INT ( 'E', 'O', 'H', '!' );
            push_pmem ( &act.final, 4, &i );    //EOH!,17

            //custom features:
            align_pmem ( &act.final );
            i = CHARS2INT ( 'T', 'I', 'T', 'L' );
            push_pmem ( &act.final, 4, &i );
            i = strlen ( scScript_Title ) + 1;
            ALIGN_INT ( i );
            push_pmem ( &act.final, 4, &i );
            pushstr_pmem ( &act.final, scScript_Title, strlen (scScript_Title) );    //title
            align_pmem ( &act.final );
            i = CHARS2INT ( 'A', 'U', 'T', 'H' );
            push_pmem ( &act.final, 4, &i );
            i = strlen ( scScript_Author ) + 1;
            ALIGN_INT ( i );
            push_pmem ( &act.final, 4, &i );
            pushstr_pmem ( &act.final, scScript_Author, strlen (scScript_Author) );   //author
            align_pmem ( &act.final );
            i = CHARS2INT ( 'E', 'N', 'D', '!' );
            push_pmem ( &act.final, 4, &i );
        }
        ...
        align_pmem ( &act.final );
        {
            ...
            while ( p )
            {
                ...
                //do the export
                {
                    ...
                    pushstr_pmem ( &act.final, p->name, strlen (p->name) );
                    align_pmem ( &act.final );
            ...
            pushstr_pmem ( &act.final, "", 0 );    //the end of exports
        }

        //Copy imports to final buffer:
        pushstr_pmem ( &act.imports, "", 0 );  //the end of imports
        align_pmem ( &act.final );
        ...
        deb2 ( "\nInit start at:%x(%d)\n", act.final.mem + act.final.pos, act.final.pos );
...
///////////////////////////////////////////////////////////////////////////
static void
scsCheckFunctionDefinitions ( char *s, scSymbol * p )
{
    while ( p )
    {
