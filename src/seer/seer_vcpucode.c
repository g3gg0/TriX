...
NameNPar opcodes_vcpu[] = {
    ...
    ,
    {"RET", 0}
    ,                           //must be the first of all nonargument
    {"WAIT", 0}
    ,                           // -
    {"CLI", 0}
...
int
ConvOpcodeTo_Double ( int opcode )
{
    switch ( opcode )
    {
        case opvcpuMOV:
...
int
FixUp_Opcode ( int opcode, _value * v1, _value * v2 )
{
    if ( opcode == -1 )
        return -1;
...
//DELS pv1=*v1;pv2=*v2;
        if ( opcode != opvcpuDFLT &&
             opcode != opvcpuFDBL && opcode != opvcpuIDBL && opcode != opvcpuFIXDBL && opcode != opvcpuDFIX && opcode != opvcpuDINT && opcode != opvcpuCMOV && opcode != opvcpuWMOV )
        {                       //Conversion allowed
...
//DELS
void
real_Gen_Opcode ( int opcode, _value * v1, _value * v2, int line, unsigned char *file )
{
	int len = 0;
	static int last_filename_ip = 0;
	static int last_srcline_ip = 0;
	static int current_line = -1;
	static unsigned char *current_file = NULL;
	static unsigned char *current_src_line = NULL;
    char cod;
    unsigned char instr[4] = { 0, 0, 0, 0 };
	_value tmp1, tmp2;

	if ( !current_file )
		current_file = strdup ( "UNKNOWN" );
	if ( !current_src_line )
		current_src_line = strdup ( "UNKNOWN" );

	if ( seer_debug_ins )
	{
		if ( file && line )
		{
			// do not mark when we should set any JMP or CALL
			if ( opcode != opvcpuJMP && opcode != opvcpuJFALSE && opcode != opvcpuJTRUE && opcode != opvcpuCALL  )
			{
				// if the filename changed or upon enter
				if ( strcmp ( current_file, file ) || opcode == opvcpuENTER )
				{
					free ( current_file );
					current_file = strdup ( file );

					real_Gen_Opcode ( opvcpuADDFILENAME, 0, 0, current_line, current_file );

				}

				if ( seer_debug_ins >= 2 )
				{
					// if the filename or line changed or upon enter
					if ( strcmp ( current_file, file ) || opcode == opvcpuENTER || current_line != line )
					{
						free ( current_src_line );
						current_src_line = strdup ( ST->lines[ST->act_line] );

						real_Gen_Opcode ( opvcpuADDSRCLINE, 0, 0, current_line, current_file );
					}
				}
				...
				if ( strcmp ( current_file, file ) || opcode == opvcpuENTER || current_line != line )
				{
					...
					scvalSetINT ( &tmp1, line );
					scvalSetINT ( &tmp2, last_filename_ip );
					real_Gen_Opcode ( opvcpuLINENUM, &tmp1, &tmp2, current_line, current_file );

					scvalSetINT ( &tmp1, line );
					scvalSetINT ( &tmp2, last_srcline_ip );
					real_Gen_Opcode ( opvcpuLINENUM2, &tmp1, &tmp2, current_line, current_file );
				}
			}
		}
	}
    ...
    if ( opcode == opvcpuADDSRCLINE )
    {
        adeb1 ( " %3d:", act.ip );
        adeb1 ( " %s", opcodes_vcpu[opcode].name );
        adeb1 ( " <%s>", current_src_line );

		Gen_Align (  );
		len = strlen ( current_src_line ) + 1;
		...
		// save string for next LINENUM opcode
		last_srcline_ip = act.code.pos; 

#ifdef _SHOW_MACHINE_CODE
        adeb0 ( "\t| " );
#endif
		push_pmem ( &act.code, len, current_src_line );
        act.ip = act.code.pos;
		Gen_Align (  );

		adeb0 ( "\n" );
	}
	else if ( opcode == opvcpuADDFILENAME )
    {
        adeb1 ( " %3d:", act.ip );
        adeb1 ( " %s", opcodes_vcpu[opcode].name );
        adeb1 ( " <%s>", current_file );
		...
		// save string for next LINENUM opcode
		last_filename_ip = act.code.pos; 

#ifdef _SHOW_MACHINE_CODE
        adeb0 ( "\t| " );
#endif
		push_pmem ( &act.code, len, current_file );
        act.ip = act.code.pos;
		Gen_Align (  );

		adeb0 ( "\n" );
	}
    else if ( opcodes_vcpu[opcode].pars == 0 )
    {
        adeb1 ( " %3d:", act.ip );
        adeb1 ( " %s ", opcodes_vcpu[opcode].name );
        cod = opcode;
    ...
    else if ( opcodes_vcpu[opcode].pars == 1 )
    {
        Gen_Align (  );
        adeb1 ( " %3d:", act.ip );
        adeb1 ( " %s ", opcodes_vcpu[opcode].name );
...
//2-st opcode
        ...
        serr2 ( scErr_CodeGen, "Code generation error %s", "for more than 2 opcodes" );
