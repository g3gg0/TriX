...
///////////////////////////////////////////////////////////////////////////
int
sctypSizeOf ( scType * type )
{
	return sctypSizeOfEx ( type, 0 );
}

int
sctypSizeOfEx ( scType * type, int basetype )
{
...
/*      {char *cq=TYPESTR;
       ...
       return i;
        ...
        if ( TYPESTR[0] == '*' || ( TYPESTR[0] == '[' && TYPESTR[1] == ']' ) )
            return 4;           //pointer
        if ( (TYPESTR[strlen(TYPESTR)-1] == '*' || ( TYPESTR[0] == '[' && TYPESTR[strlen(TYPESTR)-2] == ']' )) && basetype )
            return 4;           //pointer
        if ( TYPESTR[0] == '[' )    //TABLE
        {
            int i = 0, c = 1;
...
///////////////////////////////////////////////////////////////////////////
int
sctypInsistsAlignment ( scType * type )
{
    int i, j;
    i = sctypSizeOf ( type );
...
///////////////////////////////////////////////////////////////////////////
int
scvalFindFreeRegister ( _value * val )
{
    int il;
...
int
scvalFindFreeDoubleRegister ( _value * val )
{
    int il;
    for ( il = 2; il < 239; il++ )
    {
...
//   v->where=NULL;
    ...
    scAssert ( ArrayToPointer, imp || !( v->adr.flags & adrPOINTER ) ) if ( r )
    {
        scvalPutToRegister ( v, TRUE );
...
/* v->type.flags=typPREDEFINED;
 ...
 v->type.params=NULL;*/
    v->type.params = scStrdup ( v->type.params + 1 );
    v->type.params[0] = '*';
    deb1 ( "Converted to %s\n", v->type.params );
...
///////////////////////////////////////////////////////////////////////////
void
scvalBothTypes ( _value * v1, _value * v2 )
{                               //int typ;
...
///////////////////////////////////////////////////////////////////////////
void
scvalPutToRegister ( _value * val, int freeit )
{
...
///////////////////////////////////////////////////////////////////////////
bool
scvalIsOfType ( _value * v, valTYPE vt )
{
...
///////////////////////////////////////////////////////////////////////////
bool
scvalIsImmidiateOfType ( _value * v, valTYPE vt )
{
...
///////////////////////////////////////////////////////////////////////////
_value *
scvalSetImmidiate ( _value * v, valTYPE vt, int ival, double dval )
{
    if ( !v )
        v = New ( v );
    //address
...
///////////////////////////////////////////////////////////////////////////
_value *
scvalSetRegister ( _value * v, valTYPE vt, int reg )
{
    if ( !v )
        v = New ( v );
    //address
...
///////////////////////////////////////////////////////////////////////////
_value *
scvalSetSymbol ( _value * v, scSymbol * s )
{
    if ( !v )
        v = New ( v );
    //address
...
///////////////////////////////////////////////////////////////////////////
bool
scvalSameType ( _value * val1, _value * val2 )
{
    int v1, v2;
    ...
    if ( v1 != v2 )
    {
        if ( ( v1 == valINT && v2 == valPOINTER ) || ( v2 == valINT && v1 == valPOINTER ) )
            return true;
...
///////////////////////////////////////////////////////////////////////////
void
scvalConvToInt ( _value * val )
{
    if ( scvalGetValType ( val ) == valINT || scvalGetValType ( val ) == valPOINTER )
        return;
...
///////////////////////////////////////////////////////////////////////////
void
scvalCast ( _value * val, scSymbol * sctype )
{
...
///////////////////////////////////////////////////////////////////////////
void
sciAddImportedFunction ( scSymbol * sym, scSymbol * in_struct )
{
    int flags = 0;
    char temp[1024];
...
//do the job
    ...
    pushstr_pmem ( &act.imports, temp, strlen (temp) );
...
///////////////////////////////////////////////////////////////////////////
int
sciReadIntegerInBrackets ( int def )
{
    int imp = def;
    if ( lexeq ( "(" ) )
    {
...
///////////////////////////////////////////////////////////////////////////
int
getStructMemberFromName ( char *name, scSymbol ** in_struct )
{
    ...
    while ( strstr ( name, "." ) )
    {
        ...
        *in_struct = scdicFoundSymbol ( list, name );
        ...
        list = ( ( ( ( *in_struct )->type.flags & typTYPE ) == typSTRUCT
                   || ( ( *in_struct )->type.flags & typTYPE ) == typSCRIPT ) && ( ( *in_struct )->adr.flags & adrTYPEONLY ) ) ? ( *in_struct )->type.main : NULL;
    }
    return 1;
}
...
///////////////////////////////////////////////////////////////////////////
void
scvalAccessReference ( _value * val )
{                               //only reference
...
void
scvalDisplayValue ( char *x, _value * v )
{
    deb1 ( "%s{", x );
...
///////////////////////////////////////////////////////////////////////////
void
sctypTypeOfStr ( scType * type, char *name, char *buf )
{
    buf[0] = 0;
...
///////////////////////////////////////////////////////////////////////////
int
sciReadImport ( char **import_name )
{
    int imp = -1;
    *import_name = NULL;
    Advance;
...
///////////////////////////////////////////////////////////////////////////
scSymbol *
sctypGetStruct ( scType * typ )
{
    scSymbol *_this;
    scAssert ( sctypGetStruct, sctypGetValType ( typ ) == valSTRUCT );
    _this = typ->main;
    while ( _this && ( !( _this->adr.flags & adrTYPEONLY ) || !( _this->type.flags & typSTRUCT ) ) )
...
///////////////////////////////////////////////////////////////////////////
void
scvalConstantString ( _value * val, char *s, int len )
{
    int adr = 0;

//search for exact string already specified
    if ( act.consts.pos && len <= ( unsigned ) act.consts.pos )
    {
...
void
scvalGetAddress ( _value * val, int situ )
{
    int r = 0;
    _value vt1;
    scAssert ( scvalGetAddress, situ != 3 || ( val->adr.flags & adrADDRESS ) ) deb2 ( "adr.flags=%d&adrPOINTER=%d\n", val->adr.flags, val->adr.flags & adrPOINTER );
    if ( !( val->adr.flags & adrPOINTER ) )
    {
        deb1 ( "adr.flags=%d\n", val->adr.flags );
        switch ( situ )
