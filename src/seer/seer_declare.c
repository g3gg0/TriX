...
scReadOperatorName (  )
{
    ...
    if ( lexeq ( "==" ) || lexeq ( "<=" ) || lexeq ( ">=" ) || lexeq ( "+=" ) || lexeq ( "-=" ) ||
         ...
         lexeq ( "!=" ) || lexeq ( "<<" ) || lexeq ( ">>" ) || lexeq ( "[]" ) || lexeq ( "^=" ) || lexeq ( "&=" ) || lexeq ( "|=" ) )
    {
        name = scStrdup ( "operator??" );
        name[8] = ST->lexema[0];
        name[9] = ST->lexema[1];
    ...
    else if ( lexeq ( "(" ) )
    {
        ...
        if ( lexeq ( "int" ) )
        {
            Advance;
            if ( lexeq ( ")" ) )
                return scStrdup ( "operator(int)" );
        }
        if ( lexeq ( "double" ) )
        {
            Advance;
            if ( lexeq ( ")" ) )
                return scStrdup ( "operator(double)" );
        }
        if ( lexeq ( "char" ) )
        {
            Advance;
    ...
    while ( 1 )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                scget_back ( ST );
                if ( !ts && actu )
                    serr2 ( scErr_Declaration, "Parameter declaration expected at %s", ST->lexema );
                if ( !ts )
                {
                    scSetPosition ( sp );
                    Advance;
...
//     deb1("<%x>\n",(int)ts);
            ...
            {                   //serr( scErr_Parse,"SeeR now doesn't allow default initializers in function parameters")
                ...
                if ( ip != act.ip )
                    serr ( scErr_Declaration, "Invalid default initializer" ) sprintf ( s, "=%x", ( int ) val );
                strcpy ( temp + strlen ( temp ), s );
            }
            if ( !lexeq ( "," ) )
            {
...
#ifndef DJGPP
char *
itoa ( int v, char *s, int base )   //base =10
{
    sprintf ( s, "%d", v );
    return s;
}
...
//    deb1("temp= %s\n",temp);
            ...
            else if ( lexeq ( "." ) || lexeq ( "::" ) )
            {
                ...
                if ( ( sctypGetValType ( &structure->type ) != valSTRUCT && sctypGetValType ( &structure->type ) != valSCRIPT ) || !( structure->adr.flags & adrTYPEONLY ) )
                ...
                while ( lexeq ( "." ) || lexeq ( "::" ) )
                {
                    ...
                    else if ( !sciValidIdentifier ( ST->lexema ) )
                        serr2 ( scErr_Parse, parse_error, ST->lexema );
                    if ( lexeq ( "operator" ) )
                    {
                    ...
                    structure = ( ( ( newsym->type.flags & typTYPE ) == typSTRUCT || ( newsym->type.flags & typTYPE ) == typSCRIPT ) && ( newsym->adr.flags & adrTYPEONLY ) ) ? newsym : NULL;
                    Advance;
                }               //while
                scget_back ( ST );
            }                   //if
    ...
    if ( nested )
    {
        ADD_to_start ( temp, nested );
        scFree ( nested );
    }
    if ( strlen ( temp ) )
        nested = scStrdup ( temp );
    else
        nested = NULL;
...
//    serr(scErr_Declaration,"script cannot be a member of any function or structure");
        ...
        if ( this_symbol.reserved && ( this_symbol.type.main->type.flags & typSCRIPT || this_symbol.type.main->adr.flags & adrSCRIPT ) )
...
//      serr( scErr_Declaration,"script cannot be a member of any structure");
    ...
    {                           //struct
        ...
        if ( !lexeq ( "{" ) )   //name:
        {
            ...
            //WORK:
            if ( ( !this_symbol.type.main && ( structure = scdicFoundSymbol ( SC_types->first, ST->lexema ) ) )
            ...
            else
            {
                ...
                structure->name = scStrdup ( ST->lexema );
                structure->adr.flags = adrTYPEONLY;
                if ( this_symbol.type.main && ( this_symbol.type.main->type.flags & typSCRIPT || this_symbol.type.main->adr.flags & adrSCRIPT ) )
                {
...
//      else
            ...
            else if ( !sciValidIdentifier ( sym2->name ) )
                serr2 ( scErr_Declaration, "Invalid identifier `%s' in declaration", sym2->name );
            if ( scdicFoundSymbol ( sym->type.main, sym2->name ) )
            {
                ...
                if ( sctypIsFunction ( &sym2->type ) )
                {
                    while ( p )
                    {
                        if ( strcmp ( sym2->name, p->name ) == 0 && ( !sctypIsFunction ( &p->type ) || sctypExactFunction ( &p->type, &sym2->type ) ) )
...
//   KLS()();
        ...
        while ( i )
        {
            ...
            {
                Advance;
                sym->type.flags |= typSTATIC;
                i = 1;
            }
            if ( lexeq ( "const" ) )
            {
                Advance;
                sym->type.flags |= typCONST;
                i = 1;
            }
            if ( lexeq ( "unsigned" ) )
            {
                Advance;
                sym->type.flags |= typUNSIGNED;
                i = 1;
                if ( special_was_signed )
                    serr ( scErr_Declaration, "Declare both signed and unsigned?" );
            }
            if ( lexeq ( "signed" ) )
            {
                Advance;
                i = 1;
                special_was_signed = 1;
...
//                    deb0 ( "It is UNSIGNED!\n" );
                    ...
                    switch ( i )
                    {
                        ...
                        case valSHORT:
                            break;
                        default:
                            serr ( scErr_Declaration, "Only char,short and int can be unsigned" );
                    }
                }
                return sym;
            }
...
#endif
            sym->type.flags |= typPREDEFINED;
            sym->type.main = ( scSymbol * ) valINT; //int
            if ( WriteAlsoTo )
                *WriteAlsoTo = sym->type;
            return sym;
        }
...
#endif
                sym->type.flags |= typUSERDEFINED;
                sym->type.main = this_symbol.type.main;
                Advance;
                if ( lexeq ( "(" ) )
                {
#if DEBUGLEVEL==0
...
//disgusting gotos? without them, it wouldn't look better...
  STRUCT_DECLARE:
    while ( lexeq ( "." ) || lexeq ( "::" ) )
    {
        scSymbol *n;
        Advance;
        if ( ( sctypGetValType ( &sym->type.main->type ) != valSTRUCT && sctypGetValType ( &sym->type.main->type ) != valSCRIPT ) || !( sym->type.main->adr.flags & adrTYPEONLY ) )
...
//           ^-short(without first
scSymbol *
sctypGetShortDefinition ( scType * typ )
{
