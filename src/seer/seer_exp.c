...
int
sciReadSubExp ( int i, _value * val, bool ExpRequired )
{
            ...
            //exp1 := lvalue [=] expr | exp2      [=] =: =,+=,-=,*=,/=,%=,&=,^=,|=,<<=,>>=
        {
            ...
            else if ( lexeq ( "+=" ) )
            {
                ...
                else
                {
                    if ( scvalGetValType ( &vt1 ) == valSTRUCT )
                        serr3 ( scErr_Operand, invalid_operand_to, "s", "+=" );
                    if ( scvalIsImmidiate ( &vt1 ) && ( scvalImmidiateValue ( &vt1 ) == 0 ) )
                        return 1;
            ...
            else if ( lexeq ( "*=" ) )
            {
                ...
                else
                {
                    if ( scvalGetValType ( &vt1 ) == valSTRUCT )
                        serr3 ( scErr_Operand, invalid_operand_to, "s", "*=" );
                    if ( scvalIsImmidiate ( &vt1 ) && ( scvalImmidiateValue ( &vt1 ) == 1 ) )
                        return 1;
            ...
            else if ( lexeq ( "^=" ) )
            {
                ...
                else
                {
                    if ( scvalGetValType ( &vt1 ) == valSTRUCT )
                        serr3 ( scErr_Operand, invalid_operand_to, "s", "^=" );
                    if ( scvalIsImmidiateOfType ( &vt1, valINT ) && ( scvalImmidiateValue ( &vt1 ) == 0 ) )
                        return 1;
            ...
            else if ( lexeq ( "<<=" ) )
            {
                ...
                else
                {
                    if ( scvalGetValType ( &vt1 ) == valSTRUCT )
                        serr3 ( scErr_Operand, invalid_operand_to, "s", "<<=" );
                    if ( scvalIsImmidiateOfType ( &vt1, valINT ) && ( scvalImmidiateValue ( &vt1 ) == 1 ) )
                        return 1;
...
///////////////////////////////////////////////////////////////////////////
             ...
             */
        {
            ...
            else if ( lexeq ( "typeof" ) )
            {
                ...
                scvalConstantString ( val, styp, strlen(styp) );
            ...
            {                   //-------------------------------------------------
                ...
                if ( lexeq ( "->" ) || lexeq ( "." ) )
                {
                    ...
                    else
                    {
                        ...
                        if ( !sc )
                            serr3 ( scErr_Operand, "`%s' is not a member of `%s'", name, father->name );
                ...
                if ( lexeq ( "[" ) )
                {
                    while ( lexeq ( "[" ) )
                    {
                        ...
                        {
                            if ( !val->type.params && ( !val->type.params ||val->type.params[0] != '[' && val->type.params[0] != '*' ))
             ...
             */
        {
            ...
            if ( isdigit ( ST->lexema[0] ) )
            {
                ...
                if ( *endp )
                    serr ( scErr_Parse, "nondigits in number and not hexadecimal" );
                Advance;
                return 1;
            }
                ...
                //*************** MEMBER-SYMBOL
            if ( ( this_symbol.reserved ) && ( sym = scdicFoundSymbol ( this_symbol.type.main->type.main, ST->lexema ) ) )
            {
                _value vt1, vt2;
...
///////////////////////////////////////////////////////////////////////////
int
Read_Expression ( _value * val, bool RequiredExp )
{
//*********** expr := exp1,expr | exp1
...
///////////////////////////////////////////////////////////////////////////
void
scvalPopUsedRegisters ( int *regs )
{
    int i;
    for ( i = 239; i >= 0; i-- )
    {
...
bool
scsTypesExact ( scType * t1, scType * t2 )
{
    int gvt1, gvt2;
...
///////////////////////////////////////////////////////////////////////////
static bool
scsCanClassCast ( scType * t1, scType * t2 )
{
    int gvt1;
...
///////////////////////////////////////////////////////////////////////////
static bool
scsDoClassCast ( scType * t1, _value * v, bool doTemporary )
{
    int gvt1;
...
//t1 is formal, t2 is actual
bool
scsTypesAlmostExact ( scType * t1, scType * t2 )
{
    int gvt1, gvt2;
    ...
    if ( gvt1 == gvt2 )
    {
        switch ( gvt1 )
        {
            case valPOINTER:
            {
                ...
                sctypGoDeeper ( &ct1 );
                sctypGoDeeper ( &ct2 );
                if ( sctypGetValType ( &ct1 ) == valVOID || sctypGetValType ( &ct2 ) == valVOID )
...
///////////////////////////////////////////////////////////////////////////
bool
sctypExactFunction ( scType * t1, scType * t2 )
{
    bool after3dots = false;
    char *x1 = t1->params, *x2 = t2->params;

    if ( !x1 || !x2 )
        return false;           //returns true, if any of them is a variable
    if ( *x1 != '(' || *x2 != '(' )
        return false;           //so that we can detect name collisions
    x1++;
    x2++;
...
///////////////////////////////////////////////////////////////////////////
bool
scsFunctionOK ( scSymbol * fun, _value * params, bool ( *typeCompare ) ( scType * t1, scType * t2 ) )
{
    bool after3dots = false;
    ...
    x++;
    while ( ( params[i].adr.flags != ( adrIMPORTED & adrTYPEONLY ) ) || params[i].type.flags != typINVALID )
    {
        scSymbol *xs = NULL;
        _value *v = NULL;
...
///////////////////////////////////////////////////////////////////////////
int
scsFindFunction1Pass ( char *name, scSymbol ** found, scSymbol * find_among, scSymbol * also_find, _value * params, bool ( *typeCompare ) ( scType * t1, scType * t2 ) )
{
    int fi = 0, in_also = 0;
    scSymbol *p;
...
scSymbol *
scsFind_Function ( char *name, scSymbol * find_among, scSymbol * also_find, _value * params, char *desc )
{
    scSymbol *found[256];
    char func[256];
    int fi = 0;
    ...
    if ( fi > 1 )
    {
        ...
        serr4 ( scErr_Operand, "Unable to decide which of %d candidates for function '%s' to use:%s", fi, scStrdup ( func ), scStrdup ( buf ) );
    }
...
///////////////////////////////////////////////////////////////////////////
scSymbol *
sctypFindExactFunction ( char *name, scSymbol * find_among, scSymbol * also_find, scType * params )
{
    scSymbol *p;
    scSymbol *found = NULL;
    int in_also = 0;
...
static void
scvalPushParameters ( _value * fun, _value * params, int *size, char **param_end )
{
    int i = 0, k, l;
    scSymbol *xs = NULL;
    _value *xv = NULL;
    ...
    for ( ; i >= 0; i-- )
    {
        ...
        *size += k;
        ...
        if ( !( after3dots && i + 1 >= after3dots ) )
        {
            ...
            xs.type = formal[i];
            xs.name = NULL;
            scvalSameType ( v, scvalSetSymbol ( &temp, &xs ) );
        ...
        if ( ( ( after3dots && i + 1 >= after3dots ) || !sctypIsReference ( &formal[i] ) ) )
        {
            ...
            if ( ( after3dots && i + 1 >= after3dots ) || !scsDoClassCast ( &formal[i], v, false ) )
            {
...
*/
void
scvalCallFunction ( scSymbol * sym, _value * val, scSymbol * in_struct, _value * thisval, _value * scriptval, _value * params, bool CallerIgnore )
{
    int siz = 0;
    int ret_struct = 0;
...
//push
    ...
    if ( in_struct )
    {
        ...
        if ( ( ( in_struct->adr.address & adrTYPEONLY ) && ( in_struct->type.flags & typSCRIPT ) )
...
///////////////////////////////////////////////////////////////////////////
static bool
scsCallOperator ( char *operatorX, _value * val, _value * v1, _value * v2, _value * v3, bool CallerIgnore )
{
    int vnum = 0;
    scSymbol *father;
...
static int
scsGenStruct ( scType * stype, _value * val, scType * temptype, bool toStack )
{
    _value v1, v2;
    int ret_struct;
    int rv;
