...
void
scScript_Error ( Source_Text * ST, char *filename, int linenum, int errnum, char *er, ... )
{
    va_list arg;
//  if (scErrorNo) return;
#ifndef DEBUG
...
///////////////////////////////////////////////////////////////////////////
bool
scomit_whitespaces ( Source_Text * ST )
{
    char c;
    if ( ST->newline == -1 )
        ST->newline = 1;
    ...
    do
    {
        c = GetChar ( ST );
        if ( c == 26 )
            return false;       // end of text reached
        ST->whitespaces++;
    }
...
scget_symbol ( Source_Text * ST )
{
        ...
        if ( IsSymbol ( c ) )
        {
            ...
            *( p + 3 ) = 0;
            for ( i = 0; Special_Symbols[i]; i++ )
                if ( strcmp ( ST->lexema, Special_Symbols[i] ) == 0 )
                    return true;
        };
        UnGetChar (  );
        *( p + 2 ) = 0;
    ...
    {
        c = GetChar ( ST );
        if ( ( c > 32 && c < 127 ) && ( ( !IsSymbol ( c ) ) || ( ( c == '.' ) && isdigit ( ST->lexema[0] ) ) ) )    // printable char?
        {
            tmp = ST->lexema;
            if ( p - tmp >= DIMENSION ( ST->lexema ) - 1 )   //return false;
...
///////////////////////////////////////////////////////////////////////////
void
scGetPosition ( Source_Text * ST, scPosition * n )
{
    n->ST = ST;
    n->lin = n->ST->act_line;
    n->chr = n->ST->act_char;
...
char *escstring_err = "Unknown escape sequence";
char *charstring_err = "Character 'x' only allows one char";

///////////////////////////////////////////////////////////////////////////
////scget_back/////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
void
scget_back ( Source_Text * ST )
{
...
scget_lexema ( Source_Text * ST )
{
	...
	{                           //Read all string
		...
		temp[i] = 0;
		memcpy ( ST->lexema, temp, i );
		ST->lexema[i] = '\000';
		ST->lexema_len = i;
	}
	else if ( !parser_Comment && strcmp ( ST->lexema, "'" ) == 0 )
	{
		char temp[10];
		int val = 0;
		c=GetChar(ST);
...
//								UnGetChar ( );
		...
		c=GetChar(ST);
		if ( c != '\'' )
			serr(scErr_Parse,charstring_err);

		sprintf ( ST->lexema, "%i", val );
		ST->lexema_len = strlen ( ST->lexema);

	}
	else if ( strcmp ( ST->lexema, ".." ) == 0 )
	{
		char c;
		c = GetChar ( ST );
		if ( c != '.' )
...
///////////////////////////////////////////////////////////////////////////
char *
scget_line ( Source_Text * ST, int ignore_whitespaces )
{
    char temp[10000];
    int i = 0;
    char c = 0, last;
    if ( !STALine[ST->act_char - 1] )
        return "";
    do
    {
        last = c;
...
//             beg^   ^end
void
scSwap ( char *temp, char *source, char *toins, int beg, int end )
{
    int i;
    char *p;
    for ( i = 0; i < beg; i++ )
...
///////////////////////////////////////////////////////////////////////////
int
read_integer ( Source_Text * ST, char *s, int *integ )
{
    char *endp;
    if ( strstr ( s, "." ) )
        serr ( scErr_Parse, "Integer expected instead of double!\n" );
/*
