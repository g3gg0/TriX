...
char *hdrKernel =
    ...
    "import scScript scCompile_File(char *, int *);\n"
    "import scScript scCompile_Text(char *, int *);\n"
...
void
string_string_char ( scintString * _this, const char *x )
{
    if ( !x )
        _this->cstr = NULL;
    else
...
//" c_import string();\n"
void
string_string ( scintString * _this )
{
    _this->cstr = NULL;
#ifdef OutputDebug
    printf ( "string(%d);\n", _this );
...
scintString *
string_operatorSET_int ( scintString * _this, const int x )
{
    char temp[256];
...
void
string_string_int ( scintString * _this, const int x )
{
    _this->cstr = NULL;
...
//" c_import ~string();\n"
void
string_destroy ( scintString * _this )
{
    if ( _this->cstr )
    {
        free ( _this->cstr );
...
void
string_copy ( scintString * _this, scintString * f )
{
    _this->cstr = NULL;
    if ( f->cstr )
...
// a=x=s; <=> a.operator=(x.operator=(s));
scintString *
string_operatorSET ( scintString * _this, const scintString * f )
{
    if ( _this->cstr )
    {
#ifdef OutputDebug
...
scintString *
string_operatorSET_char ( scintString * _this, const char *f )
{
    scintString t;
    t.cstr = f;
...
// a=x+s; <=> a=x.operator+(s); <=> a.operator=(x.operator+(s));
scintString
string_operatorPLUS ( scintString * _this, const scintString * x )
{
    scintString v;
    int i, j;
    if ( NULL == _this )
    {
        fprintf ( stderr, "Fatal error in string_operatorPLUS: _this is NULL !\n" );
        exit ( 13 );
    }
    if ( NULL == x )
    {
        fprintf ( stderr, "Fatal error in string_operatorPLUS: x is NULL !\n" );
        exit ( 13 );
    }
    if ( NULL == x->cstr )
        j = 0;
...
/*
 ...
 int i;
 v.cstr=NULL;
 if (x->cstr==NULL||x->cstr[0]==0)
   return *string_operatorSET(&v,_this);
 if (_this->cstr==NULL)
   return *string_operatorSET(&v,x);
...
scintString
string_operatorPLUS_char ( scintString * _this, const char *x )
{
    scintString v;
    int i;
    if ( NULL == _this )
    {
        fprintf ( stderr, "Fatal error: in string_operatorPLUS_char _this is NULL !\n" );
        exit ( 13 );
    }
    if ( NULL == _this->cstr )
        i = 0;
...
//" c_import string operator()(int start,int len);\n"
scintString
string_sub ( scintString * _this, int start, int len )
{
    scintString v;
    int i;
    if ( NULL != _this->cstr )
...
//" c_import bool operator<?>(string& x);\n"
int
string_cmp ( const scintString * _this, const scintString * x )
{
...
//" c_import bool operator==(string& x);\n"
bool
string_eq ( const scintString * _this, const scintString * x )
{
...
//" c_import bool operator!=(string& x);\n"
bool
string_ne ( const scintString * _this, const scintString * x )
{
    return string_cmp ( _this, x ) != 0;
}
...
bool
string_eq_char ( const scintString * _this, const char *x )
{
    if ( NULL == x )
    {
        fprintf ( stderr, "string_eq_char: x is NULL !\n" );
        fflush ( stderr );
        return -1;
    }
    if ( NULL == _this->cstr )
    {
        fprintf ( stderr, "string_eq_char: _this->cstr is NULL !\n" );
        fflush ( stderr );
        return -1;
    }
...
//" c_import bool operator>(string& x);\n"
bool
string_gt ( const scintString * _this, const scintString * x )
{
    return string_cmp ( _this, x ) > 0;
}
...
//" c_import bool operator<(string& x);\n"
bool
string_lt ( const scintString * _this, const scintString * x )
{
    return string_cmp ( _this, x ) < 0;
}
...
//" c_import bool operator>=(string& x);\n"
bool
string_ge ( const scintString * _this, const scintString * x )
{
    return string_cmp ( _this, x ) >= 0;
}
...
//" c_import bool operator<=(string& x);\n"
bool
string_le ( const scintString * _this, const scintString * x )
{
    return string_cmp ( _this, x ) <= 0;
}
...
//" c_import void del(int start,int len);\n"
void
string_del ( scintString * _this, int start, int len )
{
    int j, i = 0;
    if ( _this->cstr != NULL )
        i = strlen ( _this->cstr );
...
//" c_import void insert(int start,string& x);\n"
void
string_insert ( scintString * _this, int start, scintString * x )
{
    char *c;
...
int
string_find ( scintString * _this, scintString * x )
{
    char *s;
...
//returns number of replaces
int
string_replace ( scintString * _this, scintString * what, scintString * with, bool global )
{                               //TODO
...
// a+=x; <=> a.operator+=(x);
void
string_append ( scintString * _this, scintString * x )
{
    int i;
    char *z;
#ifdef OutputDebug
...
void
string_append_char ( scintString * _this, char *x )
{
    scintString t;
#ifdef OutputDebug
    printf ( "append_C:%s\n", x );
...
//" c_import int operator(int)();\n"//toint
int
string_toInt ( scintString * _this )
{
    int i = 0;
    char *e;
...
//" c_import char operator[](int idx);\n"
char *
string_operatorIDX ( scintString * _this, int idx )
{
...
void
string_reconstruct ( scintString * _this )
{                               //nothing right now
}

int
string_length ( scintString * _this )
{
    if ( !_this->cstr )
        return 0;
...
//" c_import vector();\n"
void
vector_vectorVOID ( scintVector * _this )
{
...
//" c_import vector(double,double,double);\n"
void
vector_vector ( scintVector * _this, double x, double y, double z )
{
    _this->x = x;
    _this->y = y;
    _this->z = z;
...
//" c_import ~vector();\n"
void
vector_destroy ( scintVector * _this )
{
}

//" c_import vector& operator=(vector& v);\n"
scintVector *
vector_operatorSET ( scintVector * _this, scintVector * v )
{
    *_this = *v;
    return _this;
}
...
//" c_import vector operator+(vector& v);\n"
scintVector
vector_operatorPLUS ( scintVector * _this, scintVector * v )
{
    scintVector a;
    a = *_this;
    a.x += v->x;
...
//" c_import vector operator-(vector& v);\n"
scintVector
vector_operatorMINUS ( scintVector * _this, scintVector * v )
{
    scintVector a;
    a = *_this;
    a.x -= v->x;
...
//" c_import vector operator*(vector& v);\n"//cross-product
scintVector
vector_operatorMUL ( scintVector * _this, scintVector * v )
{
    scintVector a;
...
//" c_import void operator+=(vector& v);\n"
void
vector_operatorPLUSSET ( scintVector * _this, scintVector * v )
{
    _this->x += v->x;
    _this->y += v->y;
    _this->z += v->z;
...
//" c_import void operator-=(vector& v);\n"
void
vector_operatorMINUSSET ( scintVector * _this, scintVector * v )
{
    _this->x -= v->x;
    _this->y -= v->y;
    _this->z -= v->z;
...
//" c_import void operator*=(vector& v);\n"
void
vector_operatorMULSET ( scintVector * _this, scintVector * v )
{
...
//" c_import bool operator==(vector& v);\n"
bool
vector_operatorEQ ( scintVector * _this, scintVector * v )
{
...
//" c_import bool operator!=(vector& v);\n"
bool
vector_operatorNE ( scintVector * _this, scintVector * v )
{
...
//" c_import bool operator<(vector& v);\n"
bool
vector_operatorLT ( scintVector * _this, scintVector * v )
{
    return ( _this->x < v->x && _this->y < v->y && _this->z < v->z );
}
...
//" c_import bool operator>(vector& v);\n"
bool
vector_operatorGT ( scintVector * _this, scintVector * v )
{
    return ( _this->x > v->x && _this->y > v->y && _this->z > v->z );
}
...
//" c_import bool operator<=(vector& v);\n"
bool
vector_operatorLE ( scintVector * _this, scintVector * v )
{
    return ( _this->x <= v->x && _this->y <= v->y && _this->z <= v->z );
}
...
//" c_import bool operator>=(vector& v);\n"
bool
vector_operatorGE ( scintVector * _this, scintVector * v )
{
    return ( _this->x >= v->x && _this->y >= v->y && _this->z >= v->z );
}
...
//" c_import double length();\n"
double
__sc_vector_length ( scintVector * _this )
{
...
//" c_import void normalize();\n"
void
vector_normalize ( scintVector * _this )
{
    double length = 1.0 / __sc_vector_length ( _this );
...
//vector:
    ...
    scAdd_External_Symbol ( "vector.length", __sc_vector_length );
}
