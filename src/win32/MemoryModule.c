...
void own_crt_debugger_hook (unsigned int reserved)
{
	printf ( "Debugger hook called!\n");
}
		...
		for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++)
		{
			...
			for (; *thunkRef; thunkRef++, funcRef++)
			{
				if IMAGE_SNAP_BY_ORDINAL(*thunkRef)
				{
					...
					*funcRef = (DWORD)GetProcAddress(handle, funcName);
				}
				else 
				{
				...
				if (*funcRef == 0)
				{
					printf ( "Failed to locate symbol: '%s' in library: '%s'\n", funcName, libName);
					result = 0;
					break;
				}
			}
